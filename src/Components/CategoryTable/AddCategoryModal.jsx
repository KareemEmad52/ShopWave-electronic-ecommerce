...
import { addCategory, addProduct, getBrands, getCategories } from '../../utils/api';
import { useUser } from '../../context/UserContext';
import * as Yup from 'yup';
import { AddCategoySchema, AddProductSchema } from '../../utils/ValidationSchema';


const AddCategoryModal = ({ isOpen, closeModal, displayCategory }) => {
  const { token } = useUser();
  ...
  const handleSubmit = async (values, { resetForm }) => {
    ...
    try {
      const result = await addCategory(formData, token); // Use the addProduct function from the API module
      console.log(result);
      displayCategory()
      setIsLoading(false)
    ...
    <Fragment>
      <dialog id="my_modal_1" className="modal">
        <div className="modal-box max-w-[45rem]">
          ...
          <Formik initialValues={initialValues} onSubmit={handleSubmit} validationSchema={AddCategoySchema}>
